...
async def english_section_option(callback: CallbackQuery, state: FSMContext):
    await state.update_data(lang="english")

    section_keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Тварини",
                                                                                   callback_data="english_section_1")],
...
@router.callback_query(F.data.in_(["english_section_1"]))
async def questions_option(callback: CallbackQuery, state: FSMContext):
    if callback.data != "edit_questions_number":
        await state.update_data(section=callback.data)
...
async def english_animals_creation(callback: CallbackQuery, state: FSMContext):
    user_choice = await state.get_data()
    used_words = user_choice.get("used_words", [])

    query = EnglishAnimalsWords.select()
    if used_words:
        query = query.where(EnglishAnimalsWords.english_original.not_in(used_words))
    ...
    rand_word = random_word.english_original
    rand_word_translate = random_word.english_translated

    predicted_answers = [rand_word_translate]
    random_variants = list(EnglishAnimalsWords.select().order_by(fn.Random()).limit(5))

    for variant in random_variants:
        if len(predicted_answers) == 3:
            break

        translated_variant = variant.english_translated
